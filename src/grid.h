	...
	switch (rotStage) {
		...
		case 3:
			for (size_t i = 0; i < arrSize; ++i) {
				...
				if (activePiece->getAbsGridX(y) >= (WIDTH  )   || activePiece->getAbsGridX(y) < 0 ||
					activePiece->getAbsGridY(x) >= (HEIGHT -1) || activePiece->getAbsGridY(x) < 0 ||
					isBlock(activePiece->getAbsGridX(y),activePiece->getAbsGridY(x)))
					return false;
			}
			break;
		case 2:
